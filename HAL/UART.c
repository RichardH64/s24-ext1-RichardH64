...
UART UART_construct(uint32_t moduleInstance, uint32_t port, uint32_t pins) {
  ...
  GPIO_setAsPeripheralModuleFunctionInputPin(uart.port, uart.pins, GPIO_PRIMARY_MODULE_FUNCTION);

  // Determine what other variables need to be initialized in the config struct

  // All uart config parameters except those related to baudrate generation
...
void UART_SetBaud_Enable(UART* uart_p, UART_Baudrate baudChoice) {
  ...
  // Initialize and enable uart instance
...
bool UART_hasChar(UART* uart_p) {
  uint8_t interruptStatus = UART_getInterruptStatus(uart_p->moduleInstance, EUSCI_A_UART_RECEIVE_INTERRUPT_FLAG);

  return (interruptStatus == EUSCI_A_UART_RECEIVE_INTERRUPT_FLAG);
}

/**
 * Gets the char sent by the user
 *
 * --First the function checks if a char has been sent. If so the recieveData function
 * --of the UART is called to get the byte and is returned, else the char terminator is returned
 *
 * @param uart_p: The pointer to the UART instance with which to handle our
 * operations.
 *
 * @return byte representing the char sent by the user
 */
...
/**
 * Determines if the board can send a UART data packet to the user by checking
 * the interrupt status of the proper system UART module.
 *
 * --Gets the interrupt status with the transmit flag as the mask. Returns true if
 * --the status is equal to the transmit flag
 *
 * @param uart_p: The pointer to the UART instance with which to handle our
 * operations.
 *
 * @return true if the board can send a character, and false otherwise
 */
bool UART_canSend(UART* uart_p) {
    uint8_t interruptStatus = UART_getInterruptStatus(uart_p->moduleInstance, EUSCI_A_UART_TRANSMIT_INTERRUPT_FLAG);

  return (interruptStatus == EUSCI_A_UART_TRANSMIT_INTERRUPT_FLAG);
}

/**
 * Transmits data from the board to the user
 *
 * --Checks if the UART can send any data
 * --If it can, then UART_transmitData is called to send the char
 *
 * @param uart_p: The pointer to the UART instance with which to handle our
 * operations.
 * @param c: The char to be transmitted through the UART
 *
 * @return void
 */
